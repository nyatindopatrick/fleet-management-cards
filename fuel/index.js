...
async function submitFormResponse() {
  ...
  questionToAnswerMap[STATION] = _station;
  questionToAnswerMap[LITERS] = _liters;
  questionToAnswerMap[COST] = _cost;
...
function inflateStationDiv() {
  var question = _form.questions[STATION];

  var questionDiv = document.getElementById('stationDiv');
  ...
  for (i = 0; i < question.options.length; i++) {
    ...
    options.onclick = function () {
      _station = this.value;
      document.getElementById('SingleSelectOption' + _station).checked = true;
      invalidateFooter();
    };
...
function inflateLitersDiv() {
  var textQuestionDiv = document.getElementById('litersDiv');
  ...
  textQuestionTitle.innerText = _strings[_form.questions[LITERS].title];
...
function inflateCostDiv() {
  var textQuestionDiv = document.getElementById('costDiv');
  KASClient.UI.clearElement(textQuestionDiv);

  var textQuestionTitle = KASClient.UI.getElement('div');
  textQuestionTitle.className = 'question-title';
  textQuestionTitle.innerText = _strings[_form.questions[COST].title];
...
function inflateFooterView() {
  ...
  if (_currentPage == 1) {
    if (_registration == '' || _station == -1 || _liters == '' || _cost == '') {
...
function inflateSummaryView() {
  ...
  singleSelectQuestionDetailsHeader.innerText =
    _strings[_form.questions[STATION].title];
  ...
  singleSelectQuestion.innerHTML =
    _strings[_form.questions[STATION].options[_station].text];
  ...
  odometerDetailsHeader.innerText = _strings[_form.questions[LITERS].title];
  ...
  duedateDetailsHeader.innerText = _strings[_form.questions[COST].title];
