...
async function submitFormResponse() {
  ...
  questionToAnswerMap[STARTINGPOINT] = _startingPoint;
  ...
  if (registered.status == 400) {
    ...
    const active = await fetch(
      'https://kaizala-fleet.herokuapp.com/trips/active'
    );
    const cars = await active.json();
    if (
      cars.map((item) => item.vehicle.registrationNo).includes(_registration)
    ) {
      showError('The vehicle has an active trip!');
    } else {
...
function inflateRegistrationDiv() {
  var textQuestionDiv = document.getElementById('registerDiv');
  ...
  textQuestionTitle.innerText = _strings[_form.questions[REGISTRATION].title];
  ...
  textQuestionInput.addEventListener('blur', async (e) => {
    const registered = await fetch(
      `https://kaizala-fleet.herokuapp.com/vehicle/${e.target.value}`
    );
    if (registered.status == 400) {
      showError('The vehicle is not registered!');
    }
...
function inflateStartingPointDiv() {
  ...
  var textQuestionTitle = KASClient.UI.getElement('div');
  textQuestionTitle.className = 'question-title';
  textQuestionTitle.innerText = _strings[_form.questions[STARTINGPOINT].title];
...
function inflateSummaryView() {
  ...
  startingTripHeader.innerText = _strings[_form.questions[STARTINGPOINT].title];
