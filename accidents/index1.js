...
async function submitFormResponse() {
  ...
  questionToAnswerMap[EXTENT] = _extent;
...
function inflateOdometerDiv() {
  var question = _form.questions[EXTENT];
  ...
  for (i = 0; i < question.options.length; i++) {
    ...
    options.onclick = function () {
      _extent = this.value;
      document.getElementById('SingleSelectOption' + _extent).checked = true;
      invalidateFooter();
    };
...
function inflateSummaryView() {
  ...
  singleSelectQuestionDetailsHeader.innerText =
    _strings[_form.questions[EXTENT].title];
  ...
  singleSelectQuestion.innerHTML =
    _strings[_form.questions[EXTENT].options[_extent].text];
