...
async function submitFormResponse() {
  ...
  questionToAnswerMap[REGISTER] = _registration;
  ...
  if (registered.status == 200) {
    const datas = await registered.json();
    if (datas) {
      showError('The vehicle is already registered!');
    }
  } else {
...
function inflateRegisterDiv() {
  var registerDiv = document.getElementById('registrationDiv');
  KASClient.UI.clearElement(registerDiv);

  var registerTitle = KASClient.UI.getElement('div');
  registerTitle.className = 'question-title';
  registerTitle.innerText = _strings[_form.questions[REGISTER].title];

  var registerInput = KASClient.UI.getElement('input');
  registerInput.type = 'text';
  registerInput.className = 'comment-input';

  if (KASClient.getPlatform() == KASClient.Platform.iOS) {
    KASClient.UI.addCSS(registerInput, {
      'padding-left': '13pt',
    });
  }
  registerInput.placeholder = _strings['strTapToEnter'];
  registerInput.addEventListener('input', function (event) {
    _registration = event.target.value;
    invalidateFooter();
  });

  KASClient.UI.addElement(registerTitle, registerDiv);
  KASClient.UI.addElement(registerInput, registerDiv);
}
...
function inflateSummaryView() {
  ...
  textQuestionDetailsHeader.innerText =
    _strings[_form.questions[REGISTER].title];
  ...
  var registerQuestion = KASClient.UI.getElement('div', {
    color: '#32485f',
    'font-size': '12pt',
  ...
  registerQuestion.innerHTML = _registration;

  KASClient.UI.addElement(registerQuestion, textQuestionDetailsView);
